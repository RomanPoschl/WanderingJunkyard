...
public partial class UniverseMap : Node2D
{
	public delegate void JumpToSectorEventHandler(MapCell mapCell);
	...
	{ 
		...
		{
			var scene = _preloadedCell.Instantiate<MapCell>();
	...
	void ShowSectorInfo(Vector2 mousePosition)
	{
		...
		sectorPopupPanel.Position = new Vector2i((int)mousePosition.x, (int)mousePosition.y) ;
		sectorPopupPanel.Popup();
	}
	...
	void OnJumpButtonPressed()
	{
		...
		mapPopup.Hide();
		EmitSignal(nameof(JumpToSectorEventHandler), _currentlySelected);
	}
	...
	public override void _Process(double delta)
	{
		...
		{
			Position = _dragPosition.Value + _oldPosition;
		}
	}
}
