...
public class ShipGenerator3D
{
                ...
                for (var i = 0; i < numberOfHullSegments; i++)
                {
                    ...
                    if (val > 0.1f)
                    {
                        ...
                        if (_rng.Randf() > 0.5f)
                        {
                            ...
                            var quaterion = new Quat(new Vector3(0, 0, 1), angle);
    ...
    private Matrix4x4 GetFaceMatrix(ShipMeshFace face, Vector3? position = null)
    {
    ...
    private void AddCylindersToFace(ShipMesh ShipMesh, ShipMeshFace fac)
    {
        ...
        {
            var top = fac.LeftTop.Coordinates.LinearInterpolate(fac.RightTop.Coordinates, ((float)h + 1) / (horizontalStep + 1));
            var bottom = fac.LeftBottom.Coordinates.LinearInterpolate(fac.RightBottom.Coordinates, ((float)h + 1) / (horizontalStep + 1));

            for (var v = 0; v < verticalStep; v++)
            {
                var pos = top.LinearInterpolate(bottom, ((float)v + 1) / (verticalStep + 1));
                var cylinderMatrix = GetFaceMatrix(fac, pos) * GetRotationMatrix4x4(new Quat(new Vector3(0, 1, 0), 90));
        ...
        for (var h = 0; h < horizontalStep; h++)
        {
            ...
            for (var v = 0; v < verticalStep; v++)
            {
                ...
                var faceMatrix = GetFaceMatrix(fac, pos + fac.Normal * weaponDepth * 0.5f) *
                    GetRotationMatrix4x4(new Quat(new Vector3(0, 0, 1), _rng.RandiRange(0, 90)));
                ...
                var leftGuardMat = faceMatrix *
                    GetRotationMatrix4x4(new Quat(new Vector3(0, 1, 0), 90)) *
                ...
                var turretHouseMat = faceMatrix *
                    GetRotationMatrix4x4(new Quat(new Vector3(1, 0, 0), upwardAngle)) *
    ...
    {
        _ = Matrix4x4.Invert(GetFaceMatrix(face), out var invertedMatrix);
    ...
    public static Matrix4x4 GetTranslationMatrix4x4(Vector3 vector)
    {
    ...
    public static Matrix4x4 GetRotationMatrix4x4(Quat quat)
    {
        float x = quat.x * 2.0F;
        float y = quat.y * 2.0F;
        float z = quat.z * 2.0F;
    ...
    public static Vector3 MultiplyPoint3x4(Matrix4x4 m, Vector3 point)
    {
        Vector3 res;
